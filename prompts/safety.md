...
6. **Bounded retries.** A single locale may invoke `validate_translation` at most 3 times. After the third failure for that locale, mark it `needsReview: true` and continue with the rest of the group.
7. **No deferred writes across groups.** Commit after each key group. Do not accumulate translations in memory across multiple groups before persisting. Within a group, batch all locale updates into one `commit_bundle` call.
8. **No interactive turns.** When `next_key_group()` returns a null group, call `emit_report` once and stop. Do not ask the user for input.
